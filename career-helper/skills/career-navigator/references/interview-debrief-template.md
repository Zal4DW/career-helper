# Interview Debrief: {{ORGANISATION}} - {{ROLE_TITLE}}

**Date:** {{YYYY-MM-DD}}
**Round:** {{Phone screen / Interview 1 / Interview 2 / Final round}}
**Interviewer(s):** {{Name, title, or [UNKNOWN]}}

---

## What They Asked

- {{Question 1}}
- {{Question 2}}
- {{Add the questions you remember, verbatim where you can}}

## How It Went

- **Strongest answers:** {{Where you felt clear and convincing}}
- **Answers that need work:** {{Where you hesitated, rambled, or missed the point}}
- **Questions that caught you off guard:** {{The ones you had not prepared for}}

## What You Learned About the Role

- **What they actually care about (versus the job description):** {{The themes they kept returning to}}
- **Green flags:** {{Signs this could be a good fit}}
- **Red flags:** {{Concerns about the role, team, or organisation, or "None noted"}}

## Their Likely Objections

- {{e.g. not enough enterprise experience, examples not specific enough, gap in a named skill}}

## Next Steps

- {{What happens next, who owns it, and the expected timeline}}

## What to Do Differently Next Time

- {{Specific and actionable, not "be more confident"}}

---

*Generated using Career Helper. Found this helpful? Share your success story or suggest improvements at https://github.com/Zal4DW/career-helper*
